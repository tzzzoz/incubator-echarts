...
var calculateCandleWidth = _helper.calculateCandleWidth;
...
var _default = {
  ...
  reset: function (seriesModel) {
    ...
    return {
      progress: isLargeRender ? largeProgress : normalProgress
    };
    ...
    function largeProgress(params, data) {
      var segCount = params.end - params.start; // Structure: [sign, x1, y1, x2, y2, sign, x1, y1, x2, y2, ...]

      var points = new LargeArr(segCount * 5);

      for (var dataIndex = params.start, offset = 0, point, tmpIn = [], tmpOut = []; dataIndex < params.end; dataIndex++) {
        var axisDimVal = data.get(cDim, dataIndex);
        var openVal = data.get(openDim, dataIndex);
        var closeVal = data.get(closeDim, dataIndex);
        var lowestVal = data.get(lowestDim, dataIndex);
